import responses
from responses import matchers

from api.exchanges.coinbase_exchange import CoinbaseExchangeAdapter


@responses.activate
def test_fills_mock():
    base = "https://api-public.sandbox.exchange.coinbase.com"

    # Mock GET /fills with query params (order-independent)
    responses.add(
        responses.GET,
        f"{base}/fills",
        match=[matchers.query_param_matcher({"limit": "1", "product_id": "BTC-USD"})],
        json=[{"trade_id": 1, "product_id": "BTC-USD"}],
        status=200,
    )


    c = CoinbaseExchangeAdapter("k", "c2VjcmV0LWJhc2U2NA==", "p", base_url=base)

    data = c.fills(limit=1, product_id="BTC-USD")
    assert data[0]["product_id"] == "BTC-USD"
